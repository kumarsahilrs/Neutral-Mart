...
interface KpiProps {
  ...
  icon?: LucideIcon;
  danger?: boolean;     // render icon tile in red palette
  loading?: boolean;
}

export default function Kpi({ label, value, sub, positive, icon: Icon, danger, loading }: KpiProps) {
  const deltaColor =
    positive === true ? '#2f8049' :
    positive === false ? '#b6442a' :
    '#7a6f5d';

  const tileBg = danger ? 'var(--nm-red-soft)' : 'var(--nm-green-soft)';
  const tileFg = danger ? 'var(--nm-red)' : 'var(--nm-green)';
  ...
  return (
    <div className="nm-card" style={{ padding: '18px 20px' }}>
      <div className="flex justify-between items-center mb-3">
        ...
        {Icon && (
          <span
            ...
            style={{ width: 32, height: 32, borderRadius: 10, background: tileBg, color: tileFg }}
          >
      ...
      {loading ? (
        <div style={{ height: 28, width: 96, borderRadius: 8, background: 'var(--nm-line-soft)' }} className="animate-pulse" />
      ) : (
